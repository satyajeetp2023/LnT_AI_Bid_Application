 ...
 return <div className="mx-auto max-w-[1500px]">
  ...
  {data.summary.recorded===0?<EmptyState title="No historical results recorded yet" description="Bid Results will populate this page as tender outcomes are captured."/>:<>
   ...
   <section className="mb-3 overflow-hidden rounded border border-slate-200 bg-white">
    <div className="border-b bg-slate-50 px-4 py-3"><h2 className="text-sm font-bold text-slate-900">Historical Result Register</h2><p className="text-xs text-slate-500">Trace every summary metric back to the recorded tender result and its source evidence.</p></div>
    {data.records.length===0?<div className="p-4 text-xs text-slate-500">No historical result records match the current filters.</div>:<div className="overflow-x-auto"><table className="w-full min-w-[1100px] text-left text-xs"><thead className="bg-slate-50 text-[10px] uppercase tracking-wide text-slate-500"><tr><th className="px-4 py-2">Bid</th><th className="px-4 py-2">Client</th><th className="px-4 py-2">Project Type</th><th className="px-4 py-2">Result Date</th><th className="px-4 py-2">Result</th><th className="px-4 py-2">Our Rank</th><th className="px-4 py-2">L1</th><th className="px-4 py-2">Our Bid</th><th className="px-4 py-2">Gap to L1</th><th className="px-4 py-2">Evidence</th><th className="px-4 py-2">Source</th></tr></thead><tbody>{data.records.map(x=><tr key={x.bid_project_id} className="border-t"><td className="px-4 py-3"><a href={"/bids/"+x.bid_project_id+"/bid-results"} className="font-semibold text-[#304354] underline-offset-2 hover:underline">{x.bid_id}</a><div className="max-w-64 truncate text-[10px] text-slate-500">{x.tender_name||"—"}</div></td><td className="px-4 py-3">{x.client||"—"}</td><td className="px-4 py-3">{x.project_type||"—"}</td><td className="px-4 py-3">{x.result_date||"—"}</td><td className="px-4 py-3">{x.result_status}</td><td className="px-4 py-3">{x.our_rank??"—"}</td><td className="px-4 py-3">{x.l1_bid_value??"—"}</td><td className="px-4 py-3">{x.our_bid_value??"—"}</td><td className="px-4 py-3">{x.our_gap_to_l1_percent!==null?x.our_gap_to_l1_percent+"%":"—"}</td><td className="px-4 py-3"><span className={x.evidence_status==="Complete"?"font-semibold text-emerald-700":"font-semibold text-amber-700"}>{x.evidence_status}</span>{x.missing_evidence.length>0&&<div className="mt-1 max-w-56 text-[10px] text-slate-500">{x.missing_evidence.join(", ")}</div>}</td><td className="px-4 py-3">{x.source_reference||"—"}</td></tr>)}</tbody></table></div>}
   </section>
