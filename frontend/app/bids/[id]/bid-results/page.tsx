 ...
 return <div className="mx-auto max-w-[1500px]">
  ...
  <section className="mb-3 rounded border border-slate-200 bg-white p-3">
   <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
    <Field label="Result Status"><select value={status} onChange={e=>setStatus(e.target.value)} className="input">{["Result Awaited","Won","Lost","No Bid","Cancelled","Pending"].map(x=><option key={x}>{x}</option>)}</select></Field>
    <Field label="Result Date"><input type="date" value={resultDate} onChange={e=>setResultDate(e.target.value)} className="input"/></Field>
    <Field label="Our Rank"><input type="number" min="1" value={ourRank} onChange={e=>setOurRank(e.target.value)} className="input" placeholder="e.g. 2"/></Field>
    <Field label="Our Bid Value"><input type="number" min="0" step="0.01" value={ourBidValue} onChange={e=>setOurBidValue(e.target.value)} className="input"/></Field>
    <Field label="Our Margin %"><input type="number" step="0.01" value={margin} onChange={e=>setMargin(e.target.value)} className="input"/></Field>
    <Field label="Awarded Bidder"><input value={awardedBidder} onChange={e=>setAwardedBidder(e.target.value)} className="input"/></Field>
    <Field label="Source Reference"><input value={sourceReference} onChange={e=>setSourceReference(e.target.value)} className="input" placeholder="Tender result / LOA reference"/></Field>
    <Field label={status==="Won"?"Win Reason":"Loss / Outcome Reason"}><input value={reason} onChange={e=>setReason(e.target.value)} className="input"/></Field>
   </div>
   <div className="mt-3"><Field label="Notes"><textarea value={notes} onChange={e=>setNotes(e.target.value)} className="input min-h-20"/></Field></div>
  </section>
  ...
  <section className="mb-3 overflow-hidden rounded border border-slate-200 bg-white">
   ...
   <div className="space-y-2 p-3 md:hidden">{prices.map(row=><div key={row.rank} className="rounded border border-slate-200 p-3"><div className="mb-2 flex items-center justify-between"><div className="text-sm font-bold text-slate-900">L{row.rank}</div><label className="flex items-center gap-2 text-xs"><input type="radio" name="ours" checked={row.is_ours} onChange={()=>setPrices(rows=>rows.map(x=>({...x,is_ours:x.rank===row.rank})))} />Our Bid</label></div><div className="grid gap-2"><input value={row.bidder_name} onChange={e=>updatePrice(row.rank,{bidder_name:e.target.value})} className="input" placeholder="Bidder name"/><input type="number" min="0" step="0.01" value={row.bid_value||""} onChange={e=>updatePrice(row.rank,{bid_value:Number(e.target.value)})} className="input" placeholder="Bid value"/></div></div>)}</div>
   <div className="hidden overflow-x-auto md:block"><table className="w-full min-w-[900px] text-left text-xs"><thead className="bg-slate-50 text-[10px] uppercase tracking-wide text-slate-500"><tr>{["Rank","Bidder","Bid Value","Currency","Our Bid","Source Reference"].map(h=><th key={h} className="px-3 py-2">{h}</th>)}</tr></thead><tbody>{prices.map(row=><tr key={row.rank} className="border-t"><td className="px-3 py-2 font-bold text-slate-800">L{row.rank}</td><td className="px-3 py-2"><input value={row.bidder_name} onChange={e=>updatePrice(row.rank,{bidder_name:e.target.value})} className="input"/></td><td className="px-3 py-2"><input type="number" min="0" step="0.01" value={row.bid_value||""} onChange={e=>updatePrice(row.rank,{bid_value:Number(e.target.value)})} className="input"/></td><td className="px-3 py-2"><input value={row.currency} onChange={e=>updatePrice(row.rank,{currency:e.target.value})} className="input w-20"/></td><td className="px-3 py-2 text-center"><input type="radio" name="ours-table" checked={row.is_ours} onChange={()=>setPrices(rows=>rows.map(x=>({...x,is_ours:x.rank===row.rank})))} /></td><td className="px-3 py-2"><input value={row.source_reference||""} onChange={e=>updatePrice(row.rank,{source_reference:e.target.value})} className="input"/></td></tr>)}</tbody></table></div>
  </section>
 </div>;
}

function Field({label,children}:{label:string;children:React.ReactNode}){return <label className="block"><span className="text-[10px] font-bold uppercase tracking-wide text-slate-500">{label}</span><div className="mt-1">{children}</div></label>}
