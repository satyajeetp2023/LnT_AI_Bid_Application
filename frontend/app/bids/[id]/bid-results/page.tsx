  ...
  <section className="mb-3 overflow-hidden rounded border border-slate-200 bg-white">
   ...
   {comparison&&comparison.matches.length>0?<>
    ...
    <div className="overflow-x-auto"><table className="w-full min-w-[900px] text-left text-xs">
     <thead className="bg-slate-50 text-[10px] uppercase tracking-wide text-slate-500"><tr><th className="px-3 py-2">Bid</th><th className="px-3 py-2">Client</th><th className="px-3 py-2">Type</th><th className="px-3 py-2">Result</th><th className="px-3 py-2">Our Rank</th><th className="px-3 py-2">Gap to L1</th><th className="px-3 py-2">Similarity</th><th className="px-3 py-2">Matched On</th><th className="px-3 py-2">Result Source</th></tr></thead>
     <tbody>{comparison.matches.map(x=><tr key={x.bid_project_id} className="border-t"><td className="px-3 py-3"><div className="font-semibold text-slate-900">{x.bid_id}</div><div className="text-[10px] text-slate-500">{x.tender_name}</div></td><td className="px-3 py-3">{x.client}</td><td className="px-3 py-3">{x.project_type}</td><td className="px-3 py-3"><StatusBadge>{x.result_status}</StatusBadge></td><td className="px-3 py-3">{x.our_rank??"—"}</td><td className="px-3 py-3">{x.our_gap_to_l1_percent!==null?x.our_gap_to_l1_percent+"%":"—"}</td><td className="px-3 py-3 font-semibold">{x.similarity_score}%</td><td className="px-3 py-3">{x.matched_fields.map(v=>v.replaceAll("_"," ")).join(", ")}</td><td className="px-3 py-3"><div>{x.source_reference||"—"}</div>{x.result_date&&<div className="text-[10px] text-slate-500">{x.result_date}</div>}</td></tr>)}</tbody>
    </table></div>
    {comparison.competitors.length>0&&<div className="border-t p-3">
     <div className="mb-2 text-xs font-bold text-slate-800">Competitors in Comparable Bids</div>
     <div className="overflow-x-auto"><table className="w-full min-w-[620px] text-left text-xs"><thead className="bg-slate-50 text-[10px] uppercase tracking-wide text-slate-500"><tr><th className="px-3 py-2">Competitor</th><th className="px-3 py-2">Appearances</th><th className="px-3 py-2">L1 Wins</th><th className="px-3 py-2">L1 Rate</th><th className="px-3 py-2">Avg Rank</th></tr></thead><tbody>{comparison.competitors.map(x=><tr key={x.name} className="border-t"><td className="px-3 py-2 font-semibold text-slate-900">{x.name}</td><td className="px-3 py-2">{x.appearances}</td><td className="px-3 py-2">{x.l1_wins}</td><td className="px-3 py-2">{x.l1_rate_percent}%</td><td className="px-3 py-2">{x.average_rank}</td></tr>)}</tbody></table></div>
    </div>}
