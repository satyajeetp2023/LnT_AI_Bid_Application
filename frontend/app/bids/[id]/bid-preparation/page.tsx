   ...
   <div className="max-h-[650px] overflow-auto border-t">
    <table className="w-full min-w-[1100px] text-left text-xs">
     ...
     <tbody>{plan.rows.map(row=><tr key={`${row.sheet}-${row.clause_reference}`} className="border-t align-top">
      ...
      <td className="px-3 py-3"><div className="space-y-1.5">{row.fields.filter(f=>f.action!=="preserve").map(f=><div key={`${f.coordinate}-${f.semantic_field}`} className="rounded bg-slate-50 p-2"><div className="flex flex-wrap items-center gap-2"><span className="font-semibold text-slate-700">{f.coordinate||"—"} · {f.header||f.semantic_field}</span><span className={f.action==="propose_auto_fill"?"rounded bg-emerald-50 px-1.5 py-0.5 text-[9px] font-bold text-emerald-700":f.action==="employer_only"?"rounded bg-slate-200 px-1.5 py-0.5 text-[9px] font-bold text-slate-600":"rounded bg-amber-50 px-1.5 py-0.5 text-[9px] font-bold text-amber-700"}>{f.action.replaceAll("_"," ")}</span></div>{f.proposed_value!==null&&f.proposed_value!==""&&<div className="mt-1 font-medium text-slate-800">Proposed: {f.proposed_value}</div>}{f.reason&&<div className="mt-1 text-[10px] leading-4 text-slate-500">{f.reason}</div>}</div>)}</div></td>
     </tr>)}</tbody>
    </table>
   </div>
  </section>}
