...
from app.services.pre_bid_queries import approve_pre_bid_query,create_pre_bid_query,list_pre_bid_queries,pre_bid_query_summary,revoke_pre_bid_query_approval,update_pre_bid_query
...
def revoke_query_approval(query_id:int,request:Request,db:Session=Depends(get_db),user:User=Depends(user_dep)):
 item=get_item(db,query_id);require_project_access(db,user,item.bid_project_id,Permission.PRE_BID_QUERY_APPROVE);return revoke_pre_bid_query_approval(db,item,user.id,metadata(request))
