...
def test_upload_duplicate_classify_notes_archive_and_filter(client,bid_payload):
 ...
 classified=client.patch(f"/api/v1/documents/{doc['id']}/classification",json={"document_category":"Technical Specification","document_subcategory":"OHE","information_tags":["Safety","OHE"]});assert classified.status_code==200 and classified.json()["information_tags"]==["Safety","OHE"]
 assert client.patch(f"/api/v1/documents/{doc['id']}/notes",json={"notes":"Reviewed"}).json()["notes"]=="Reviewed"
 found=client.get(f"/api/v1/bids/{bid['id']}/documents",params={"category":"Technical Specification","search":"spec"}).json();assert found["total"]==1
